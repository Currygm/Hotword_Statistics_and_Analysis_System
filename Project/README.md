...
# -o: 输出文件
# -s: 滑动窗口步长
# -m: 分词模式 (Cut(HMM), Cut(NoHMM), CutForSearch, TF-IDF, TextRank)
# -u: 用户自定义词典路径
# -w: 额外停用词路径
# -k: UDP发送的最大Top-K数量
...
*   **分词模式**：选择以下五种模式之一：
    *   `Cut(HMM)`：基于HMM的混合分词，新词识别能力强（默认推荐）。
    *   `Cut(NoHMM)`：不使用HMM的快速分词，速度快但对新词识别弱。
    *   `CutForSearch`：搜索引擎模式，分词粒度更细，适合搜索场景。
    *   `TF-IDF`：**最先进技术** — 使用TF-IDF算法，从每行文本中提取权重最高的关键词（每行最多10个），再统计这些关键词的词频。适合新闻、博客等长文本，过滤掉高频但语义权重低的词（如"的""了"等），热词结果更具语义价值。
    *   `TextRank`：**最先进技术** — 使用基于图算法的TextRank方法，从每行文本中提取关键词（每行最多10个），再统计词频。适合文章摘要场景，能识别词语在文本结构中的重要性。

    > **注意**：`TF-IDF` 和 `TextRank` 模式每行只提取Top-10关键词参与计数，因此热词频次会低于分词模式，但词语质量更高。
*   **自定义词典配置 (折叠菜单)**：
